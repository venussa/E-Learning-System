 ...
 */
	...
	class update_data extends load{
		...
		function save_data(){
			...
			foreach ($join_data as $key => $value) {
				...
				if($key == "birth_day"){
					...
					if($this->check_email($value, $build["where-nim"]) == false){
				...
				}else $build["where-nim"] = (int) clean_xss_string($value);

			}

			API_access("data_update?name=data_student", true, $build);

			$path = SERVER."/sources/repository/".$build["where-nim"];
			if(!file_exists($path)){
				mkdir($path);
		...
		function check_email($email = null, $nim){
