		...
		function save_data(){
			...
			foreach ($join_data as $key => $value) {
				...
				if($key == "nidn"){
					...
					if($this->check_nidn($value) == false){
			...
			API_access("data_insert?name=data_teacher", true, $build);

			$path = SERVER."/sources/repository/".$build['nidn'];
			if(!file_exists($path)){
				mkdir($path);
